-- =============================================================================
-- DEMO-ONLY · Signup → seed clone
-- =============================================================================
-- Run on the demo Supabase project AFTER 01_multitenancy.sql.
--
-- When someone signs up at demo.vars.live, Supabase Auth inserts a row into
-- auth.users. The trigger below fires on that insert and copies a mini
-- portfolio (2 buildings, 7 units, 4 residents, 1 guard, 2 vendors, sample
-- invoices, a couple of visits + service requests) into rows owned by the
-- new user. They land on the dashboard with a populated app rather than
-- a blank slate.
--
-- The function is SECURITY DEFINER so it can insert into RLS-protected
-- tables on behalf of a user who hasn't received their first JWT yet.
-- =============================================================================
...
DECLARE
  ...
  -- Profile ids (residents + 1 guard + self)
  p_self    uuid := v_uid;                 -- the signing-up user (PMC)
  ...
  -- Derived display name from email local-part: "ahmed.k@x.ae" -> "Ahmed K"
  v_display text := initcap(replace(replace(split_part(NEW.email, '@', 1), '.', ' '), '_', ' '));
BEGIN
  -- 0. Self profile (PMC) — owner_id of the user themselves is themselves.
  ...
  -- 2. Units (3 floors x 3 units + 1 villa). Mix occupied / vacant.
  ...
  -- 3. Resident profiles + assignments
  ...
  -- 6. Invoices (mix of paid / pending / overdue)
  ...
  -- 7. Visits today (one pre-approved, one on-premise)
...
EXCEPTION WHEN OTHERS THEN
  -- Never block signup on a seed failure. The user gets an empty app
  -- rather than a 500. Logs go to Postgres logs.
  RAISE WARNING 'seed_demo_user_portfolio failed for %: %', NEW.id, SQLERRM;
  RETURN NEW;
END;
$$;

-- Wire the trigger. Drops first so re-running this file is safe.
